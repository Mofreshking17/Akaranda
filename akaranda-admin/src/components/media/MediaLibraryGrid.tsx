...
export default function MediaLibraryGrid({ files }: { files: MediaFile[] }) {
  ...
  if (files.length === 0) {
    return <p className="text-center text-neutral-400 py-16">No files in this folder yet.</p>;
  }

  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
      {files.map((file) => (
        <div key={file.id} className="group relative border border-neutral-200 rounded-md overflow-hidden bg-white">
          ...
          <p className="text-xs text-neutral-600 px-2 py-1.5 truncate">{file.file_name}</p>
          ...
          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
            <button
              ...
              title="Copy URL"
            >
              {copiedId === file.id ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4 text-neutral-700" />}
            </button>
            <button
              onClick={() => remove(file)}
